...
export default function PlayStationPage() {
  const psProducts = products.filter((item) => item.category === "playstation");

  return (
    <main className="min-h-screen bg-white pt-24 px-4 sm:px-8">
      <h1 className="text-3xl sm:text-4xl font-bold text-blue-600 mb-8">PlayStation Products</h1>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {psProducts.map((product) => (
